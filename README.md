...
Because extracting landmarks from all ~21 000 WLASL video clips takes longer than a single
Kaggle session (~12 h), the notebook [`kaggle/landmark_extraction.ipynb`](kaggle/landmark_extraction.ipynb)
is designed to run across **multiple sessions**, each one continuing exactly where the last left off.
...
3. In **Cell 0** set:
   ```python
   PREV_SESSION_SLUG = ""   # no previous session
   PREV_DATASET_DIR  = ""
   ```
...
3. In **Cell 0** set:
   ```python
   PREV_SESSION_SLUG = "last-hope-s1"   # match the dataset slug exactly
   PREV_DATASET_DIR  = ""               # leave empty for auto-detection
   ```
...
### Non-standard Kaggle dataset mount paths

When you add a dataset **from another Kaggle user**, Kaggle may mount it at a
non-standard path:

| Source | Mount path |
|--------|-----------|
| Your own dataset | `/kaggle/input/{slug}/` |
| Another user's dataset | `/kaggle/input/datasets/{owner}/{slug}/` |

**Cell 0 auto-detects both formats.** If the slug is found at either location it
will print:

```
[CONFIG] Auto-detected PREV_DATASET_DIR: /kaggle/input/datasets/pandk24/last-hope-s1
```

If auto-detection fails (e.g. the slug was renamed), set `PREV_DATASET_DIR`
directly in Cell 0:

```python
PREV_SESSION_SLUG = "last-hope-s1"                                     # informational only
PREV_DATASET_DIR  = "/kaggle/input/datasets/pandk24/last-hope-s1"      # full path override
```
...
It will **raise a `FileNotFoundError`** with a detailed message (including a listing
of all available datasets, including non-standard mounts) if `PREV_DATASET_DIR`
points to a path that does not exist, so failures are never silent.
...
|---------|-------------|-----|
| `[RESTORE] npy in working: 0` | Wrong dataset slug or dataset not attached | Check `PREV_SESSION_SLUG` and the Kaggle input panel |
| `FileNotFoundError: Previous dataset directory not found` | Dataset slug typo or non-standard mount path | The error message lists available datasets — set `PREV_DATASET_DIR` to the correct path |
| `[CONFIG] WARNING: dataset slug '…' not found` | Dataset attached but at a non-standard path | Set `PREV_DATASET_DIR` directly, e.g. `/kaggle/input/datasets/{owner}/{slug}` |
